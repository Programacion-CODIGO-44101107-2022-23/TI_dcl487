...
No, previamente hay que cargarlas en MC. La CPU solo ejecuta instrucciones cargadas  en Memoria Central.
...
RESPUESTA:
	CPU -> Controlar a los demás componentes de la computadora e indicar en cada momento lo que hay que hacer
...
RESPUESTA:
		...
		GPU -> órdenes gráficas
	        ...
	        “Drivers” -> programas que controlan el funcionamiento de cada dispositivo (software dependiente del dispositivo)
...
RESPUESTA:
Únicamente el lenguaje máquina.
No es portable --> cada computadora (CPU) tiene su propio repertorio de instrucciones máquina  (lenguaje máquina) que es distinto de una CPU a otra de un modelo diferente.
...
Compilador: Traduce el programa fuente generando un programa ejecutable cuya ejecución es independiente del compilador.
